...
%token mc_import bib_io bib_lang mc_out signe_s signe_f signe_d mc_afficher
...
;	
Nom_BIB:bib_io {bib_io_declare = 1;}
       |bib_lang {bib_lang_declare = 1;}
;
...
Partie_ins: ins Partie_ins
          |mc_afficher pvg {afficherTableSymbole();}
;
...
;
mc_out2 :  {if(!bib_io_declare) printf("Bibliothèque isil.io manquante pour l'éxecution du programme.\n");
}
;
input: mc_in mc_in2 par_ouv in_exp par_frm pvg_inout 
;
mc_in2 :  {if(!bib_io_declare) printf("Bibliothèque isil.io manquante pour l'éxecution du programme.\n");
}
;
...
;
IDFS: idf vrg IDFS {   if(desempiler($1)==0 && error==0) { printf("Erreur format ou nombre de formatage dans la fonction IN/OUT à la ligne: %d.\n",nb_ligne); error=1;  }
                      
                      if(!estDeclare($1)) printf("Identificateur %s non declaré à la ligne %d.\n",$1,nb_ligne);}
| idf {   if(desempiler($1)==0 && error==0 ) {printf("Erreur format ou nombre de formatage dans la fonction IN/OUT à la ligne: %d.\n",nb_ligne); error=1; }
          if(!estDeclare($1)) printf("Identificateur %s non declaré à la ligne %d.\n",$1,nb_ligne);}
;
...
;
exp: quote chaine quote         { if((getVarType()!='s')&&(conflit==0)) {printf("Conflit de types à la ligne: %d.\n",nb_ligne); conflit=1;}}
  ...
  | val operateur exp {if((getVarType()=='d')&&(getValType()!='d')&&(conflit==0)) {printf("Conflit de types à la ligne: %d.\n",nb_ligne); conflit=1;}
                                 if((getVarType()=='s')&&(conflit==0)) {printf("Conflit de types à la ligne: %d.\n",nb_ligne); conflit=1;}}
	
  | val {if((getVarType()=='d')&&(getValType()!='d')&&(conflit==0)) {printf("Conflit de types à la ligne: %d.\n",nb_ligne); conflit=1;}
                                   if((getVarType()=='s')&&(conflit==0)) {printf("Conflit de types a la ligne: %d.\n",nb_ligne); conflit=1;}}

  | idf_exp divi exp {if((getVarType()=='s')&&(conflit==0)) {printf("Conflit de types à la ligne: %d. \n",nb_ligne); conflit=1;}
...
idf_exp:                           idf        { if(!estDeclare($1)) printf("Identificateur %s non declaré à la ligne %d.\n",$1,nb_ligne);
                                                if((getVarType()=='d')&&(typeEntite($1)!='d')&&(conflit==0)) {printf("Conflit de types à la ligne: %d. \n",nb_ligne); conflit=1;}
                                                if((getVarType()=='s')&&(typeEntite($1)!='s')&&(conflit==0)) {printf("Conflit de types à la ligne: %d. \n",nb_ligne); conflit=1;}}
               ...
               | idf_Tab cr_ouv cst cr_frm {   if(depassementIndexTableau($1,$3)) printf("Dépassement de taille du tableau %s à la ligne %d.\n",$1,nb_ligne);
                                               ...
                                               if((getVarType()=='s')&&(typeEntite($1)!='s')&&(conflit==0)) {printf("Conflit de types à la ligne: %d \n.",nb_ligne); conflit=1;}}
operateur: plus 
          | moins {if(getVarType()=='s') printf("Conflit de types, opération - est interdite sur les chaines! à la ligne: %d.\n",nb_ligne);}
          | mult {if(getVarType()=='s') printf("Conflit de types, opération - est interdite sur les chaines! à la ligne: %d.\n",nb_ligne);}
